	...
	switch (mode) {
		...
		case 'plan':
			return [
				...
				'You may use tools **Read**, **Glob**, **Grep**, **LSP** (language-server ops on **filePath** + **operation**, only if an LSP server is available via plugins or legacy `lsp.servers`), **ListMcpResourcesTool** / **ReadMcpResourceTool**, and **ask_plan_question** (multiple-choice clarification). You do NOT have Write, Edit, or **Bash** — the app does not register them in Plan mode.',
		...
		case 'agent':
			return [
				...
				'- **LSP**: Language-server ops on **filePath**; servers come from **plugin** folders (`<data>/plugins/` or `<workspace>/.async/plugins/`) using `.lsp.json` or `plugin.json` `lspServers` (Claude Code–style `extensionToLanguage`), plus optional legacy **settings.json** `lsp.servers`, and bundled **typescript-language-server** for TS/JS when present. Operations: **goToDefinition**, **findReferences**, **hover**, **documentSymbol**, **workspaceSymbol**, **goToImplementation**, **prepareCallHierarchy**, **incomingCalls**, **outgoingCalls**, **getDiagnostics** (1-based **line**/**character** except **getDiagnostics**/**workspaceSymbol**).',
